...
use fj_conformance::decision_ledger::{
    DecisionLedgerOutputPaths, decision_ledger_summary_json, write_decision_ledger_outputs,
};
...
impl Args {
    fn parse() -> Result<Self, String> {
        ...
        let defaults = DecisionLedgerOutputPaths::for_root(&root);
        Ok(Self {
            root,
...
fn main() {
    ...
    let report = match write_decision_ledger_outputs(&args.root, &args.report, &args.markdown) {
        Ok(report) => report,
        Err(err) => {
            eprintln!("error: failed to write decision ledger outputs: {err}");
            std::process::exit(1);
        }
    };
    ...
    println!(
        "decision ledger gate: status={} rows={} calibration_buckets={} stale_calibration={} divergences={} issues={}",
        report.status,
        report.summary.row_count,
        report.summary.calibration_bucket_count,
        report.summary.stale_calibration_count,
        report.summary.strict_hardened_divergence_count,
        report.issues.len(),
    );
    match serde_json::to_string_pretty(&decision_ledger_summary_json(&report)) {
        Ok(raw) => println!("{raw}"),
...
fn write_forensic_log(
    args: &Args,
    report: &fj_conformance::decision_ledger::DecisionLedgerReport,
) -> Result<(), std::io::Error> {
    ...
    let mut log = E2EForensicLogV1::new(
        "frankenjax-cstq.19",
        "e2e_decision_ledger_gate",
        "fj_decision_ledger_gate",
    ...
    log.inputs = serde_json::json!({
        "required_decision_classes": report.required_decision_classes,
    ...
    log.expected = serde_json::json!({
        "gate_status": "pass",
        "required_decision_classes": report.summary.required_decision_class_count,
        "stale_calibration": 0,
        "red_drift_rows": 0,
        "unallowed_divergence": 0,
    });
    log.actual = decision_ledger_summary_json(report);
    log.tolerance = E2ETolerancePolicy {
        policy_id: "exact_decision_ledger_calibration".to_owned(),
        atol: None,
        rtol: None,
        ulp: None,
        notes: Some(
            "Decision ledger auditability is exact: rows need alternatives, loss matrices, evidence signals, posterior/confidence, calibration bucket, artifact refs, dashboard row, and replay command."
                .to_owned(),
        ),
    };
    log.error = E2EErrorClass {
        ...
        taxonomy_class: if report.status == "pass" {
            ...
            "decision_ledger_calibration".to_owned()
        },
    };
    log.artifacts = vec![
        artifact_ref(
            &args.root,
            &args.report,
            "decision_ledger_calibration_report",
        )?,
        artifact_ref(
            &args.root,
            &args.markdown,
            "decision_ledger_calibration_markdown",
        )?,
    ];
    log.replay_command = stable_replay_command(args);
    if status.requires_failure_summary() {
        log.failure_summary = Some(
            "decision ledger gate found missing evidence, stale calibration, bad loss matrix, unallowlisted divergence, missing artifact refs, or redaction-policy violations"
                .to_owned(),
        );
    }
    log.metadata.insert(
        "bead".to_owned(),
        serde_json::json!({
            "id": "frankenjax-cstq.19",
            "title": "Prove decision/evidence ledger calibration and auditability"
        }),
    );
...
fn stable_replay_command(args: &Args) -> String {
    let defaults = DecisionLedgerOutputPaths::for_root(&args.root);
    let mut parts = vec!["./scripts/run_decision_ledger_gate.sh".to_owned()];
...
fn print_usage() {
    eprintln!(
        "Usage: fj_decision_ledger_gate [--root <repo>] [--report <json>] [--markdown <md>] [--e2e <json>] [--enforce]"
    );
}
